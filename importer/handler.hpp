...
private:
    ...
    void write_node() {
        ...
        if(m_node_tracker.next_is_same_entity()) {
            valid_to = next.timestamp().to_iso();
        }
    ...
    void write_way() {
        const osmium::Way& next = m_way_tracker.next();
        const osmium::Way& cur = m_way_tracker.cur();

        if(m_debug) {
            std::cout << "way w" << cur.id() << 'v' << cur.version() << " at tstamp " << cur.timestamp() << " (" << cur.timestamp().to_iso() << ")" << std::endl;
        }
    ...
    void write_way_to_db(
        ...
        if(geom == NULL) {
            ...
            if(m_way_tracker.prev_is_same_entity()) {
                ...
                const osmium::Way& prev = m_way_tracker.prev();

                bool looksLikePolygon = PolygonIdentifyer::looksLikePolygon(prev.tags());
                geom = m_geom.forWay(prev.nodes(), prev.timestamp().seconds_since_epoch(), looksLikePolygon);
